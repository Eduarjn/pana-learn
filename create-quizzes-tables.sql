-- Script para criar tabelas de quiz no Supabase Dashboard
...
-- Verificar se a tabela quizzes já existe
SELECT 'Verificando tabela quizzes...' as info;
SELECT EXISTS (
...
-- Verificar se a tabela quiz_config existe
SELECT 'Verificando tabela quiz_config...' as info;
SELECT EXISTS (
  SELECT FROM information_schema.tables 
  WHERE table_schema = 'public' 
  AND table_name = 'quiz_config'
) as quiz_config_exists;
...
-- Inserir quiz para o primeiro curso ativo
...
WHERE table_schema = 'public' 
AND table_name IN ('quizzes', 'quiz_perguntas', 'progresso_quiz')
ORDER BY table_name;
...
SELECT q.id, q.titulo, q.nota_minima, COUNT(qp.id) as total_perguntas
FROM public.quizzes q
LEFT JOIN public.quiz_perguntas qp ON q.id = qp.quiz_id
GROUP BY q.id, q.titulo, q.nota_minima;
