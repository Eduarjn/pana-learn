-- Script SUPER SIMPLES - SEM PALAVRA CATEGORIA
...
CREATE INDEX IF NOT EXISTS idx_quizzes_tipo_curso ON public.quizzes(tipo_curso);
...
CREATE INDEX IF NOT EXISTS idx_certificados_tipo_curso ON public.certificados(tipo_curso);
...
-- Inserir quiz para tipos de curso básicos
INSERT INTO public.quizzes (tipo_curso, titulo, descricao, nota_minima)
VALUES 
...
-- Inserir perguntas de exemplo para o quiz Omnichannel
INSERT INTO public.quiz_perguntas (quiz_id, pergunta, opcoes, resposta_correta, explicacao, ordem)
SELECT 
  q.id,
  ...
  1
FROM public.quizzes q
WHERE q.tipo_curso = 'Omnichannel'
ON CONFLICT DO NOTHING;

-- Inserir perguntas de exemplo para o quiz VoIP
INSERT INTO public.quiz_perguntas (quiz_id, pergunta, opcoes, resposta_correta, explicacao, ordem)
SELECT 
  q.id,
...
SELECT q.id, q.tipo_curso, q.titulo, q.nota_minima, COUNT(qp.id) as total_perguntas
FROM public.quizzes q
LEFT JOIN public.quiz_perguntas qp ON q.id = qp.quiz_id
GROUP BY q.id, q.tipo_curso, q.titulo, q.nota_minima;

-- Comentários para documentação
COMMENT ON TABLE public.quizzes IS 'Quizzes configurados para cada tipo de curso';
